...
//&*&*&*SJ1_20100820, Modify TI Fastboot USB Vender ID.
#if 0
...
#else
/* This is the Android Sooner USB vendor id */
#define DEVICE_VENDOR_ID  0x18D1
/* This is just made up.. */
#define DEVICE_PRODUCT_ID 0xDEED
/* This is just made up.. */
#define DEVICE_BCD        0x0311;
#endif
//&*&*&*SJ2_20100820, Modify TI Fastboot USB Vender ID.
...
//&*&*&*20101201_Peter ++
int inand_update_mode_preboot(void)
{
#if (defined(CONFIG_TWL4030_KEYPAD) && (CONFIG_TWL4030_KEYPAD))
	int i;
	unsigned char key1, key2;
...
//		printf("keys=0x%x, key1=0x%x, key2=0x%x\n", keys, key1, key2);
		if ((1 == CFG_FASTBOOT_PREBOOT_KEYS) &&
		    (1 == keys)) {
			if (CFG_INAND_UPDATE_MODE_PREBOOT_KEY1 == key1)
				return 1;
		} else if ((2 == CFG_FASTBOOT_PREBOOT_KEYS) &&
			   (2 == keys)) {
			if ((CFG_INAND_UPDATE_MODE_PREBOOT_KEY1 == key1) &&
			    (CFG_INAND_UPDATE_MODE_PREBOOT_KEY2 == key2))
				return 1;
		}
		udelay(CFG_FASTBOOT_PREBOOT_LOOP_WAIT);
	}
