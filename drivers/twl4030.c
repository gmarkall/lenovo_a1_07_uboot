...
/***********add by joe for the recovery model************/
...
//      int ret;
//      ret = 0;
//      u8 value_r,value_c,value_ret,value;
//      value_r = 0;
//      value_c = 0;
        ...
        for(cb = 0; cb < 3; cb++){
                ...
                twl4030_i2c_write_u8(TWL4030_CHIP_KEYPAD, c, KEYPAD_KBC_REG);
        //      twl4030_i2c_read_u8(TWL4030_CHIP_KEYPAD, &cc, KEYPAD_KBC_REG);
        //      printf(">>>>>>>>>>>>>>>>>>cc:%x<<<<<<<<<<<<<<<<<<\n",cc);
                twl4030_i2c_read_u8(TWL4030_CHIP_KEYPAD, &r ,KEYPAD_KBR_REG);
      //        printf("r*************:%x\n", r);
                if(r == 0xfd)
...
{
  int key;
  if ((key = twl4030_keypad_read_volume_key())) 
  {
    return key;
  }
  else
