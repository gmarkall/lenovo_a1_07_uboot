...
U_BOOT_CMD(mmcinit, 6, 1, do_mmc,
	...
	" <controller[0/1]>\n"
	"mmc <controller[0/1]> read <RAM addr> <MMC start_sect> <MMC size 0x????????>\n"
	"mmc <controller[0/1]> write <RAM addr> <MMC start_sect> <MMC size 0x????????>\n"
	"mmc <controller[0/1]> erase <MMC start_sect> <MMC size 0x????????>\n");
#endif  /* CFG_CMD_MMC */
